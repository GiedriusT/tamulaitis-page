...
import { AstHeadingNode, AstNode } from './common';
...
const processHeading = (node: AstHeadingNode) => {
  ...
  node.type = 'html';
  node.value = `<h1>${title || ''}${subtitle ? `<span class="separator"> - </span><span class="subtitle">${subtitle}</span>` : ''}</h1>`;
